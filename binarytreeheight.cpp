...
int binarytreeheight(node*);
int main ()
{
    srand(time(0));int n=9;int a[n+1];int height; 
    node* start=NULL;  
    for(int i=0;i<=n;i++)
    {
    ...
    display (start,0);height = binarytreeheight(start);
    cout<<endl<<"height of tree = "<<height<<endl;    
}
int binarytreeheight(node* np)
{
   ...
   {
     leftheight=binarytreeheight(np->left);
     rightheight=binarytreeheight(np->right);
